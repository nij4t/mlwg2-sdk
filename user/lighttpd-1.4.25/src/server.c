...
static void sigaction_handler(int sig, siginfo_t *si, void *context) {
	...
	case SIGALRM: 
		handle_sig_alarm = 1; 
		break;
    case SESSIONCHECK: check_session = 1; break;
    case LOGINFAIL: login_fail = 1; break;
    case USERMODIFY: user_modify = 1; break;
	case SIGHUP:
		/** 
...
# endif
    ...
    sigaction(SESSIONCHECK, &act, NULL);
    sigaction(LOGINFAIL, &act, NULL);
    sigaction(USERMODIFY, &act, NULL);
    ...
    signal(SESSIONCHECK, signal_handler);
    signal(LOGINFAIL, signal_handler);
    signal(USERMODIFY, signal_handler);
#endif
	...
	//Gemtek 
	int pid=0;
	int fork_child=4; //create 4 child process
	while(fork_child > 0 && (pid=fork()) != 0)
	{
		usleep(100);
		fork_child=fork_child-1;
	}
	//Gemtek end
	while (!srv_shutdown) {
		int n;
