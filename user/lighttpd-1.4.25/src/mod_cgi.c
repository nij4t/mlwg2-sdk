...
int json_check(const char *str)
{
        ...
        if(!obj)
                ret = 0;
        else if(tok->err != json_tokener_success)
                ret = 0;
	else
		ret = 1;
...
static int cgi_demux_response(server *srv, handler_ctx *hctx) {
	...
	while(1) {
		...
		if (con->file_started == 0) {
			...
			//log_error_write(srv, __FILE__, __LINE__, "sss", "response header:[", hctx->response_header->ptr, "]");	
			...
			#ifdef LIGHTTPD_SUPPORT_JSON
			if(is_header_end == 0) {
				if(json_check(hctx->response_header->ptr)) { 
					is_header_end = 1;
					is_header = 0;
				}
			}
