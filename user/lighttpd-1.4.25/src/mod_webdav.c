...
static int webdav_copy_file(server *srv, connection *con, plugin_data *p, physical *src, physical *dst, int overwrite) {
	...
	int i=0;
	int count=0;//Gemtke add for security usb
	...
	if (-1 == (fd = open(src->path->ptr, O_RDONLY | O_BINARY))) {
		close(ofd);
		return 403;
	}
	...
	{
		//Gemtke add for security usb
		if(!strncasecmp(src->path->ptr,"/media/USB",10))
		{
			count++;
			check_security_usbfun_webdav_copy(con,count);
			if(count >= 10)
				count=0;
		}
		//Gemtke add end
		if (-1 == write(ofd,buf,i)) {
			switch(errno) {
			case ENOSPC:
				status = 507;
				break;
...
#else
					//Gemtek add for fixed copy subfolder issue
					if (0 != (status = webdav_copy_dir(srv, con, p, &s, &d, overwrite))) {
						break;
					}
					//Gemtek end, USE_PROPPATCH not define need add this
#endif
				}
...
int check_security_usbfun_webdav(connection *con) //0:stop 1:pass 2:login pass
{
		system("security_usb_login.sh check");
		if(!strcmp(nvram_safe_get("current_security_usb_in"),"0") || !strcmp(nvram_safe_get("current_security_usb_in"),"2"))
		{
			return 1;
		}
		else
...
int check_security_usbfun_webdav_copy(connection *con,int count) //0:stop 1:pass 2:login pass
{
		//system("security_usb_login.sh check");
		if(!strcmp(nvram_safe_get("current_security_usb_in"),"0") || !strcmp(nvram_safe_get("current_security_usb_in"),"2"))
		{
        ...
        if(fp != NULL)
        {
            ...
            while(fgets(buff, 1024, fp) != NULL)
            {
                if (strncasecmp(buff,access_ip,strlen(access_ip))==0 && buff[strlen(access_ip)]==' ' && count>=10)
                {
                    fclose(fp);
                    memset(buff, 0x00, sizeof(buff));
                    sprintf(buff,"security_usb_login.sh time %s &",access_ip);                    
...
URIHANDLER_FUNC(mod_webdav_subrequest_handler) {
	...
	//Gemtek add for security usb
	if(!strncasecmp(con->physical.path->ptr,"/media/USB",10))
	{
		int pass=check_security_usbfun_webdav(con);
		if(pass==0)
		{
			con->http_status = 403;
	...
	case HTTP_METHOD_PROPFIND:
		...
		//Gemtek opendir fail, change to depth = 0
		...
		case 1:
			if (NULL != (dir = opendir(con->physical.path->ptr))) {
				...
				while(NULL != (de = readdir(dir))) {
					...
					//Gemtek add for security_usb
					if(strlen(de->d_name)>=3 && !strcmp(con->physical.path->ptr,"/media/") && !strncasecmp(de->d_name,"USB",3))
					{
						int pass=check_security_usbfun_webdav(con);
						if(pass==0)
							continue;			
	...
	case HTTP_METHOD_PUT: {
		...
		int ofd=0;
		/*
		if (NULL != (ds_range = (data_string *)array_get_element(con->request.headers, "X-Expected-Entity-Length"))) 
		{
			int len;
			char buf[1024*64]={0};
			long long int value=0;								
								
			value=strtoll(ds_range->value->ptr,NULL,10);
			...
			while(value>0)
			{
				...
				if(len>0)
				{
					...
					for(i=0;i<len;i++)
					{
						if(buf[i]==0x0D && buf[i+1]==0x0A && buf[i+6]==0x0D && buf[i+7]==0x0A
							&& isdigit(buf[i+2]) && isdigit(buf[i+3]) && isdigit(buf[i+4]) && isdigit(buf[i+5]))
						{
							for(j=i;j<len-8;j++)
								buf[j]=buf[j+8];
							for(j=len-8;j<len;j++)
					...
					ofd = open(con->physical.path->ptr,O_WRONLY | O_APPEND | O_CREAT);
					write(ofd, buf, len);					
					close(ofd);
					value=value-len;
				}
