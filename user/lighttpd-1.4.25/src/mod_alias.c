...
PHYSICALPATH_FUNC(mod_alias_physical_handler) {
	...
	for (k = 0; k < p->conf.alias->used; k++) {
		...
		#ifdef LIGHTTPD_SUPPORT_JSON
		/* for cgi (not only for json)
 		lighttpd.conf includes "*.cgi=>"/lighttpd_www/cgi-bin", 
		when parser get "*.cgi", it will add "cgi-bin/" to the path of cgi.
 		*/
		if(!strcmp(ds->key->ptr, "*.cgi")) {
			if(strstr(uri_ptr, ".cgi") && !strstr(uri_ptr, "cgi-bin/")) {
				buffer_copy_string_buffer(con->physical.basedir, ds->value);
				buffer_copy_string_buffer(srv->tmp_buf, ds->value);
				buffer_append_string(srv->tmp_buf, uri_ptr);
				buffer_copy_string_buffer(con->physical.path, srv->tmp_buf);

				log_error_write(srv, __FILE__, __LINE__, "sss", "uri", "url.alias: ", con->physical.path->ptr);
