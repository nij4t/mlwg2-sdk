...
/* Jayo 2009/08/27: For not assign "not lease ip" to clients */
#define GT_NOT_LEASE_IP
...
#define LEASES_FILE		"/tmp/udhcpd.leases" /* Jimmy : default lease file*/	
...
struct not_lease_ip_range
{
        uint32_t start_val;
        uint32_t end_val;
        struct not_lease_ip_range *next;
};
#endif
...
#ifdef GT_NOT_LEASE_IP
	struct not_lease_ip_range *not_lease_list_ptr;	/* Jayo 2009/08/27 : for recording the range of not leased ip */
#endif
};
...
struct dhcpOfferedAddr {
    ...
    // 20070328 Jimmy add below: for dump dhcp option 60
    uint8_t vendor[255];
    // 20070328 Jimmy add above
};

// 20070328 Jimmy modify left: for dump dhcp option 60
struct dhcpOfferedAddr *add_lease(uint8_t *hostname, const uint8_t *chaddr, uint32_t yiaddr, unsigned long lease, uint8_t *vendor_string);
