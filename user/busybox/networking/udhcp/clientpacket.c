...
int send_renew(uint32_t xid, uint32_t server, uint32_t ciaddr)
{
	...
	/* Jayo 2010/03/23: add for DHCP options */
	uint16_t dhcp_max_size = 0x0224;
	add_simple_option(packet.options, DHCP_MAX_SIZE, htons(dhcp_max_size));
	add_simple_option(packet.options, DHCP_LEASE_TIME, htonl(lease_seconds));
	add_param_req_option(&packet);
#ifndef GT_NEW_LOG
