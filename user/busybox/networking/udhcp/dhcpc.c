...
{
	/* Jayo 2009/1/13 : set DHCP State */
	nvram_set("DHCPState", "2"); //renewing
	...
	switch (state) {
	case BOUND:
		/* Jayo 2010/10/05: change listen mode to LISTEN_RAW for fixing WCR-GN bug#16818*/
		//change_listen_mode(LISTEN_RAW);
		/*Alex 2011/12/08:#53366 , #53720 */
		  change_listen_mode(LISTEN_KERNEL);
	case RENEWING:
	case REBINDING:
		state = RENEW_REQUESTED;
#ifdef GT_NEW_LOG
		addLog(_LOGINFO,"Entering REBINDING state...") ; /* log */
#endif
		break;
...
#ifdef GT_NEW_LOG
		addLog(_LOGINFO,"Entering INIT state...") ; /* log */
#endif
		break;
	case INIT_SELECTING:
		break;
...
{
	/* Jayo 2009/1/13 : set DHCP State */
	nvram_set("DHCPState", "3"); //stopping
	...
	if (state == BOUND || state == RENEWING || state == REBINDING) {
		...
		{
			char log_info[128] = {'\0'};
			snprintf(log_info, sizeof(log_info), "Unicasting a release of %s to %s",inet_ntoa(temp_addr), buffer);
		...
		//udhcp_run_script(NULL, "deconfig");
		udhcp_run_script(NULL, "release");
	}
#ifdef GT_NEW_LOG
	addLog(_LOGINFO,"Entering RELEASED state") ; /* log */
#else
	...
	/* Jayo 2009/1/13 : set DHCP State */
	nvram_set("DHCPState", "4"); //stopped
	/* Derek 2009/01/16 : Turn off internet link LED */
	//system("/usr/sbin/led 3 off");
...
/* The rest has variable bit positions, need to be clever */
...
#ifdef GT_NEW_LOG
	setLogLevel(_LOGDBG,"DHCPC");  /* set log level to debug */
	addLog(_LOGINFO, "udhcpc (v"BB_VER") started") ; /* log */
#else	
	...
	if (!client_config.vendorclass)
		...
		client_config.vendorclass = alloc_dhcp_option(DHCP_VENDOR, "Kingston MLWG2", 0);
	...
	for (;;) {
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
				if (packet_num < discover_retries) {
					...
					/* Jayo 2009/1/13 : set DHCP State */
					nvram_set("DHCPState" ,"0"); //querying
#ifdef GT_NEW_LOG
					{
						char log_info[128] = {'\0'};
						snprintf(log_info, sizeof(log_info), "Sending DISCOVER ...count(%d)", packet_num+1);
						addLog(_LOGINFO, log_info) ; /* log */
					}
#endif
...
#ifdef GT_NEW_LOG
				addLog(_LOGINFO,"DHCPC timeout...") ; /* log */
#endif
				/* Jayo 2009/1/13 : set DHCP State */
				nvram_set("DHCPState", "4"); //stopped
				/* Derek 2009/01/16 : Turn off internet link LED, turn off internet error LED */
				//system("/usr/sbin/led 3 off");
				//system("/usr/sbin/led 4 on");


#if BB_MMU /* -b is not supported on NOMMU */
				if (opt & OPT_b) { /* background if no lease */
#ifdef GT_NEW_LOG
					addLog(_LOGINFO,"No lease, forking to background.") ; /* log */
#else
...
#ifdef GT_NEW_LOG
					addLog(_LOGINFO,"No lease, failing.") ; /* log */
#else
			...
			case REQUESTING:
				if (packet_num < discover_retries) {
					...
					{
#ifdef GT_NEW_LOG
						addLog(_LOGINFO, "Sending RENEW...") ; /* log */
#endif
					...
					else /* broadcast */
					{
#ifdef GT_NEW_LOG
						{
							...
							temp_in_addr.s_addr = requested_ip;
							sprintf(log_info,"Sending SELECT for %s...count(%d)",
								          inet_ntoa(temp_in_addr),packet_num+1);
							addLog(_LOGINFO,log_info) ; /* log */
						}
#endif
...
#ifdef GT_NEW_LOG
				addLog(_LOGINFO,"DHCPC timeout...") ; /* log */
				addLog(_LOGINFO,"Lease lost, entering INIT state...") ; /* log */
#endif
				continue;
			case BOUND:
		  	...
		  	change_listen_mode(LISTEN_KERNEL);
#ifdef GT_NEW_LOG
				addLog(_LOGINFO,"Lease is starting to run out, entering RENEWING state...") ; /* log */
				addLog(_LOGINFO,"Entering RENEWING state...") ; /* log */
#else
...
#endif
					/* send a request packet */
					send_renew(xid, server_addr, requested_ip); /* broadcast */
					timeout >>= 1;
					continue;
				}
		...
		if (listen_mode != LISTEN_NONE && FD_ISSET(sockfd, &rfds)) {
...
#ifdef GT_NEW_LOG
					addLog(_LOGINFO,"Received DHCP OFFER...") ; /* log */
					addLog(_LOGINFO,"Enter REQUESTING state...") ; /* log */
#endif
			...
			case REBINDING:
				if (*message == DHCPACK) {
#ifdef GT_NEW_LOG
					addLog(_LOGINFO,"Received DHCP update...") ; /* log */
#endif
					temp = get_option(&packet, DHCP_LEASE_TIME);
					if (!temp) {
#ifdef GT_NEW_LOG
						addLog(_LOGINFO, "No lease time with ACK, using 1 hour lease");
#else
...
#ifdef GT_NEW_LOG
                        char log_info[128] = {'\0'};
                        snprintf(log_info, sizeof(log_info), "Lease of %s obtained, lease time %ld"
                                        , inet_ntoa(temp_addr), lease_seconds);
                        addLog(_LOGINFO,log_info) ; /* log */
#else
					...
					/* Jayo 2009/1/13 : set DHCP State */
					nvram_set("DHCPState", "1"); //communicating
...
#ifdef GT_NEW_LOG
					addLog(_LOGINFO,"Received DHCP NAK...") ; /* log */
#else
		 ...
		 * But it wasn't a packet. It's a signal pipe then. */
		{
			...
			case SIGTERM:
#ifdef GT_NEW_LOG
				addLog(_LOGINFO,"Received SIGTERM...") ; /* log */
				addLog(_LOGINFO,"udhcpc EXIT...") ; /* log */
#else
