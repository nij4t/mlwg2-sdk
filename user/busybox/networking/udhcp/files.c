...
/* Jayo 2009/08/27 : Add the not lease IP to link list*/
#ifdef GT_NOT_LEASE_IP
static void IPstring2range(char *ip_string, uint32_t *range_start, uint32_t *range_end)
{
        *range_start = 0;
        *range_end = 0;
        char* temp_ip_string = NULL;

        if(strchr(ip_string,'-') != NULL)
        {
                temp_ip_string = strtok(ip_string,"-");
                uint32_t net_addr_t = inet_addr(temp_ip_string);
                if(net_addr_t != -1)
                        *range_start = ntohl(net_addr_t);
                else
                {
                        //ERROR: is invalid IP address format!
                        printf("ERROR: Invalid IP Address Format! \n");
                }

                temp_ip_string = strtok(NULL,"-");

                net_addr_t = inet_addr(temp_ip_string);
                if(net_addr_t != -1)
                        *range_end = ntohl(net_addr_t);
                else
                {
                        //ERROR: is invalid IP address format!
                        printf("ERROR: Invalid IP Address Format! \n");
        ...
        else
        {
                ...
                uint32_t net_addr_t = inet_addr(ip_string);
                if(net_addr_t != -1)
                        *range_start = ntohl(net_addr_t);
                else
                {
...
static int read_not_lease_ip(const char *const_line, void *arg)
{
	char *line;
	uint32_t ip_range_start = 0, ip_range_end = 0;

	//Read IP range
	line = (char *) const_line;
	//ip_string = strtok(line, "\t");
	IPstring2range(line, &ip_range_start, &ip_range_end);
	//add new data to data structure
	addNotLeaseIP(arg, ip_range_start, ip_range_end);
...
static const struct config_keyword keywords[] = {
	...
	/* ADDME: static lease */
#ifdef GT_NOT_LEASE_IP
	/* Jayo 2009/08/27 : add a new handler for not leased ip */
	{"not_lease_ip", read_not_lease_ip, &(server_config.not_lease_list_ptr), ""},
#endif
};
...
void write_leases_forClientMonitor(void)
{
	int fp;
	unsigned i; 
	time_t curr = time(0);
	...
	fp = open_or_warn("//tmp//udhcpd.leases_forClientMonitor", O_WRONLY|O_CREAT|O_TRUNC);
	if (fp < 0) {
		return;
	}
	...
	if (server_config.notify_file) {
// TODO: vfork-based child creation
		char *cmd = xasprintf("%s %s", server_config.notify_file, "//tmp//udhcpd.leases_forClientMonitor");
		system(cmd);
		free(cmd);
	}
	...
	while (i < server_config.max_leases
	 && full_read(fp, &lease, sizeof(lease)) == sizeof(lease)
		...
		if (y >= server_config.start_ip && y <= server_config.end_ip) {
			...
			// 20070328 Jimmy modify below
			if (!(add_lease(lease.hostname, lease.chaddr, lease.yiaddr, lease.expires, lease.vendor))) {
