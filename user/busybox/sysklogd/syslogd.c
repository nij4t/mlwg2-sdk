...
/* Jayo 2008/12/16 : For log type selecting of Remote logging */
#if ENABLE_FEATURE_REMOTE_LOG 
#define REMOTE_LOG_SELECT
#endif

#ifdef REMOTE_LOG_SELECT
#include "nvram_lib.h"
#include <string.h>
static unsigned char RlogAddrTln   = 0;
static unsigned char RlogIPFilter  = 0;
static unsigned char RlogFirewall  = 0;
static unsigned char RlogPPPoE     = 0;
static unsigned char RlogDDNS      = 0;
static unsigned char RlogDHCPC     = 0;
static unsigned char RlogDHCPS     = 0;
static unsigned char RlogAOSS      = 0;
static unsigned char RlogWIRELESS  = 0;
static unsigned char RlogAUTH      = 0;
static unsigned char RlogCONFIGURE = 0;
static unsigned char RlogBOOT      = 0;
static unsigned char RlogNTP       = 0;
static unsigned char RlogWIRED     = 0;

/* Jayo 2008/12/16 : get Remote Log Selecting setting   */
static void getRLogSelectSetting(void)
{
    if(!strcmp((char*)nvram_get("RlogAddrTln"), "1"))
        RlogAddrTln = 1;

    if(!strcmp((char*)nvram_get("RlogIPFilter"), "1"))
        RlogIPFilter = 1;

    if(!strcmp((char*)nvram_get("RlogFirewall"), "1"))
        RlogFirewall = 1;

    if(!strcmp((char*)nvram_get("RlogPPPoE"), "1"))
        RlogPPPoE = 1;

    if(!strcmp((char*)nvram_get("RlogDDNS"), "1"))
        RlogDDNS = 1;

    if(!strcmp((char*)nvram_get("RlogDHCPC"), "1"))
        RlogDHCPC = 1;

    if(!strcmp((char*)nvram_get("RlogDHCPS"), "1"))
        RlogDHCPS = 1;

    if(!strcmp((char*)nvram_get("RlogAOSS"), "1"))
        RlogAOSS = 1;

    if(!strcmp((char*)nvram_get("RlogWIRELESS"), "1"))
        RlogWIRELESS = 1;

    if(!strcmp((char*)nvram_get("RlogAUTH"), "1"))
        RlogAUTH = 1;

    if(!strcmp((char*)nvram_get("RlogCONFIGURE"), "1"))
        RlogCONFIGURE = 1;

    if(!strcmp((char*)nvram_get("RlogBOOT"), "1"))
        RlogBOOT = 1;

    if(!strcmp((char*)nvram_get("RlogNTP"), "1"))
        RlogNTP = 1;

    if(!strcmp((char*)nvram_get("RlogWIRED"), "1"))
        RlogWIRED = 1;
}

/* Jayo 2008/12/16 : Judge the log message is selected log type or not  */
static int isSelectedLogType(char *msg)
{
    if( RlogIPFilter  && strstr(msg, "FILTER") != NULL)
        return 1;
    if( RlogFirewall  && strstr(msg, "FIREWALL") != NULL)
        return 1;
    if( RlogDHCPC     && strstr(msg, "DHCPC") != NULL)
        return 1;
    if( RlogDHCPS     && strstr(msg, "DHCPS") != NULL)
        return 1;
    if( RlogAddrTln   && strstr(msg, "NAT") != NULL)
        return 1;
    if( RlogPPPoE     && strstr(msg, "PPPOE") != NULL)
        return 1;
    if( RlogWIRELESS  && strstr(msg, "WIRELESS") != NULL)
        return 1;
    if( RlogWIRED     && strstr(msg, "WIRED") != NULL)
        return 1;
    if( RlogBOOT      && strstr(msg, "BOOT") != NULL)
        return 1;
    if( RlogNTP       && strstr(msg, "NTP") != NULL)
        return 1;
    if( RlogAUTH      && strstr(msg, "AUTH") != NULL)
        return 1;
    if( RlogCONFIGURE && strstr(msg, "CONFIGURE") != NULL)
        return 1;
    if( RlogAOSS      && strstr(msg, "AOSS") != NULL)
        return 1;
    if( RlogDDNS      && strstr(msg, "DDNS") != NULL)
        return 1;
...
/* Jayo : If they have specified remote logging, then get remote logging selceting setting  */
#ifdef REMOTE_LOG_SELECT
	getRLogSelectSetting();
#endif
