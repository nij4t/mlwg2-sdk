...
//			"Can't bind socket. Port in use or insufficient privilege\n");
			"Can't bind socket. Port in use or insufficient privilege port=%d\n",port);
		*err = getSocketError();
		return INVALID_SOCKET;
	}
...
*/
//+++Eric add to get client ip AM 11:39 2008/3/14
	nvram_set("ssl_client_ip",inet_ntoa(addr.sin_addr));
//---Eric add to get client ip AM 11:38 2008/3/14
	setSocketNodelay(fd);
	setSocketBlock(fd);
	return fd;
}
...
//+++Eric Modified PM 05:25 2008/1/16
	char *lan_ip;
	lan_ip = nvram_get("lan_ipaddr");
	
//	sprintf( ipbuf ,"%s", lan_ip );
