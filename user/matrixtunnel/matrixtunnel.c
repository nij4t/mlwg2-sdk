...
*/
	...
	ncp->plain_up=1;
	ncp->secure_up=1;
	ILOG("Connection established. %d => %d", ncp->plain, ncp->secure->fd);	
	...
	*/
	// fork here
		...
		while (!quit) {
			...
			if(res<0) {
				//+++ GTK February 13, 2008.
				/*
				perror("select");
				*/
				perror("GTK, proxyAccept(), matrixtunnel.c, select");
				//--- GTK
				continue;
			}
/*
...
//			perror("GTK, main(), matrixtunnel.c, select");
			//--- GTK
			continue;
		}
...
//+++ Eric add PM 09:22 2008/1/21
/*
	 nvram set matrixtunnel
		 0:Default 
		 1:Remote SSL(Port is manually set.)
		 2:LAN SSL(Port 443)
		 3:LAN None SSL(Port 80)
		 4:Remote None SSL(Port is manually set.)
*/


			if(srv_port == 443){
				nvram_set("client_from_remote", "0");
				nvram_set("matrixtunnel", "2");
			}
			else{
				nvram_set("client_from_remote", "1");
				nvram_set("matrixtunnel", "1");
			}		
