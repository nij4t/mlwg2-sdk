...
struct daemon *read_opts (int argc, char **argv)
{
	  ...
	  if (!fgets(buff, MAXDNAME, f))
	    {
	      ...
	      fclose(f);
          f = NULL;	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4493
	      if (file_save)
		{
		  /* may be nested */
	  ...
	  switch (option)
		...
		if (!arg)
		...
		else 
		  {
			...
			else	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4492
			  {
			free(dhcp_file);	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4491
				if (comma)	
					free(dhcp_sname);	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4492
			  }
		  }
  ...
  if (daemon->options & OPT_RESOLV_DOMAIN)
	...
	if(f != NULL)	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4493
		fclose(f);
