...
struct frec {
  ...
  unsigned int reply_count, srv_fail_count;	/* for filter server failure, send server failure only when all servers fail. Ben */
  unsigned int rcode;		// Ben 961005
  int fd, forwardall;
  unsigned int crc;
  time_t time;
  struct frec *next;
...
struct daemon {
  ...
  unsigned server_count; /* DNS server count, for filtering server failure reply. Ben */
  int cachesize;
  int port, query_port;
