 ...
 rerun:
  ...
  for (q=0; q<qdcount; q++)
	...
	return 0; /* bad packet */
      ...
      if(name)
      {
      	if((qtype == T_A) && strstr(name,"mobilelite."))
      	{
      		//ans = 1 ;
      		sprintf(name,"%s",nvram_get("LanIPAddr"));
						struct in_addr addr;
            unsigned long ttl = 0;
            char *ipaddr = nvram_get("LanIPAddr");
            ...
            memcpy(ansp,&addr,INADDRSZ);
            ansp+=INADDRSZ;
            anscount++;
            header->qr = 1; // response 
            header->aa = 1; // authoritive - never 
            header->ra = 1; // recursion if available 
...
/* return zero if we can't answer from cache, or packet size if we can */
int redirect_request(HEADER *header, char *limit, unsigned int qlen, struct daemon *daemon, 
		   struct in_addr local_addr, struct in_addr local_netmask, time_t now) 
{
  char *name = daemon->namebuff;
  unsigned char *p, *ansp, *pheader;
 ...
 rerun:
  ...
  for (q=0; q<qdcount; q++)
	...
	return 0; /* bad packet */
      ...
      unsigned short type = T_A; /* IPv4 */
      unsigned long ttl = 1;     /* Time to live is 1 sec. */
      {
	char lan_addr[16] = {'\0'};
        strncpy(lan_addr, (char *)nvram_get("LanIPAddr"), sizeof(lan_addr));
	inet_aton(lan_addr, &(addr.addr.addr4));
      }
