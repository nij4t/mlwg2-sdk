...
{
	void *handle = NULL;	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4475
	char *error;
	char *new_name = NULL;	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4477
	char *lname = NULL;		// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4476
	struct iptables_target *m;
	...
	if (!handle) {
		...
		if (!handle) {
			...
			printf("\n");
			if(new_name)	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4477
				free(new_name);
			if(lname)	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4476
				free(lname);
			return NULL;
		}
	}
		...
		if ((error = dlerror()) != NULL) {
			...
			if (NULL == m) {
				...
				if (NULL == m) {
					...
					if(new_name)	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4477
						free(new_name);
					if(lname)	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4477
						free(lname);
					return NULL;
				}
			}
	...
	if(handle)	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4475
		dlclose(handle);
	if(new_name)	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4477
		free(new_name);
	if(lname)	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4476
