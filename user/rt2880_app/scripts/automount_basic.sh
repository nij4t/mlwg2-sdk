...
exit 1
	...
	dev_rm_buf=`hdparm -g /dev/$1 | grep "sectors = 0"`
	if [ -e "/dev/"$1"1" ]; then
		echo "***** $1 no need *****"
		exit 0
	elif [ "$dev_rm_buf" != "" ]; then
		echo "***** $1 no need2 *****"
		exit 0
	else
		echo "***** $1 basic*****" 
...
do
	...
	echo "***** SD Card *****"
	error_dir=`mount | grep /media/SD_Card1 | wc -l`
	if [ "$error_dir" == "1" ]; then
		sambaset `ls /media`
		killall lighttpd
		lighttpd -f /usr/lighttpd.conf
		if ! umount "/media/SD_Card1"; then
			sambaset `ls /media` &
			exit 1
		fi
		if ! rmdir "/media/SD_Card1"; then
			sambaset `ls /media` &
			exit 1
		fi
	fi
	if [ -d "/media/SD_Card1" ]; then
		exit 1
	fi
	media_name="SD_Card1" 
	fi
	...
	echo "***** USB *****"
	error_dir=`mount | grep /media/USB1 | wc -l`
	if [ "$error_dir" == "1" ]; then
		sambaset `ls /media`
		killall lighttpd
		lighttpd -f /usr/lighttpd.conf
		if ! umount "/media/USB1"; then
			sambaset `ls /media` &
			exit 1
		fi
		if ! rmdir "/media/USB1"; then
			sambaset `ls /media` &
			exit 1
		fi
	fi
