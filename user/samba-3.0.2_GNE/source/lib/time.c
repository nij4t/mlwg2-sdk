...
time_t get_create_time(SMB_STRUCT_STAT *st,BOOL fake_dirs)
{
  ...
  ret1 = MIN(ret, st->st_atime);
	...
	// Gemtek +++ 20110325 --- for some file types (e.g. .txt), 
	//	Windows will get wrong "create time" "00:00:00 but correct date" in file info
	struct tm *atimes = localtime(&st->st_atime);
	res = atimes->tm_sec + atimes->tm_min + atimes->tm_hour;
	// debug message is at /tmp/log/samba.log
	//DEBUG(0, ("asec=%d\n", atimes->tm_sec));
	//DEBUG(0, ("amin=%d\n", atimes->tm_min));
	//DEBUG(0, ("ahour=%d\n", atimes->tm_hour));

  if(ret1 != (time_t)0 && res != 0)
    return ret1;
