...
#if 0
/* 
	Call user_update.  If for any reason this fails, just ignore the error 
	(other than logging). 
 GEW*/
static void user_update(	const char * action, 
							const char * user_name, 
							const char * function_name )
{
	int ret;
	pstring buf; /* a 'pstring' is more than big enough to hold the command line. */
	int offset;

DEBUG(1, ("user_update: called from %s, user_name='%s'\n", function_name, user_name));

	/* Build user_update command. */		
	pstrcpy( buf, "/sbin/user_update " );
	pstrcat( buf, action );
	pstrcat( buf, " " );
	pstrcat( buf, user_name );
	
	/* We need to escape the dollar sign. */
	offset = strlen( buf ) - 1;
	if (buf[ offset ] == '$')
	{
		buf[ offset ] = '\\';
...
DEBUG(1, ("user_update: calling '%s'\n", buf));

	/* Execute user_update. */
	ret = smbrun(buf, NULL);
	if (ret != 0)
	{
		DEBUG(0,("user_update: Call to user_update failed (%d)\n", ret));
	}	
}
#endif
...
//  user_update( "ADD", newpwd->smb_name, __FUNCTION__ ); /*GEW*/
...
//  user_update( "MODIFY", pwd->smb_name, __FUNCTION__ ); /*GEW*/
...
//  user_update( "DELETE", name, __FUNCTION__ ); /*GEW*/
