...
#include "includes.h"
//#define USB_LED_ON system("echo 0 > /proc/GPIO7;echo 1 > /proc/GPIO8;");
//#define USB_LED_OFF system("echo 1 > /proc/GPIO7;echo 1 > /proc/GPIO8;");
#define USB_LED_ON
#define USB_LED_OFF
...
void LED_counter()
{
	...
	if(nvram_safe_get("usb_led_count") != "")
		count = atoi(nvram_safe_get("usb_led_count"));
	else
		count = 0;

/*
	if(nvram_safe_get("usb_led_limit") != "")
		limit = atoi(nvram_safe_get("usb_led_limit"));
	else
		limit = 10;
*/
	limit = 150;
//value = simple_itoa(limit);
//nvram_set("test_limit",value);

	if(count >= limit)
	{
//		system("echo 1 > /proc/GPIO7");// LED OFF
		nvram_set("usb_led_count", "0");
//		system("echo 0 > /proc/GPIO7");//LED ON
//	system("sh /tmp/usb_led_command&");
		if( !strcmp(nvram_safe_get("usb_error_led"), "0") )
			system("sh /tmp/usb_led_command&");
	}
	else
	{
		count++;
		nvram_set("usb_led_count", simple_itoa(count));
	}
...
static SMB_OFF_T seek_file(files_struct *fsp,SMB_OFF_T pos)
{
	...
	if(seek_ret == -1) {
		DEBUG(3,("seek_file: (%s) sys_lseek failed. Error was %s\n",
...
tryagain:
	...
	DEBUG(3,("read_file (%s): pos = %.0f, size = %lu, returned %lu\n",
...
//DEBUG(0,("@@@@write_file 4"));
	...
	DEBUG(3,("write_file (%s)(fd=%d pos=%.0f size=%u) wcp->offset=%.0f wcp->data_size=%u\n",
	 ...
	 */
//DEBUG(0,("@@@@write_file 5"));
	if (wcp->data_size) {

		BOOL cache_flush_needed = True;
   ...
   +-------------------+
    ...
    Case 3).
		  ...
		  */
			...
			DEBUG(3,("write_file: non cacheable write : fd = %d, pos = %.0f, len = %u, current cache pos = %.0f \
len = %u\n",fsp->fd, (double)pos, (unsigned int)n, (double)wcp->offset, (unsigned int)wcp->data_size ));
			...
			if ( n <= wcp->alloc_size && n > wcp->data_size) {
				...
				if ((pos <= wcp->offset) &&
					...
					DEBUG(3,("write_file: discarding overwritten write \
...
//DEBUG(0,("@@@@write_file 7"));
	...
	if (n) {
		...
		DEBUG(3,("wcp->offset = %.0f wcp->data_size = %u cache return %u\n",
...
static BOOL setup_write_cache(files_struct *fsp, SMB_OFF_T file_size)
{
	...
	if((wcp = (write_cache *)malloc(sizeof(write_cache))) == NULL) {
		DEBUG(3,("setup_write_cache: malloc fail.\n"));
		return False;
	}
	...
	if((wcp->data = malloc(wcp->alloc_size)) == NULL) {
		DEBUG(3,("setup_write_cache: malloc fail for buffer size %u.\n",
...
ssize_t flush_write_cache(files_struct *fsp, enum flush_reason_enum reason)
{
	...
	size_t data_size;
	ssize_t ret;
DEBUG(10,("<<<<Enter flush write cache !!>>>>"));
if(!wcp)
	DEBUG(10,("<<<<flush write cache 1>>>>"));
	...
	DEBUG(3,("flushing write cache: fd = %d, off=%.0f, size=%u\n",
