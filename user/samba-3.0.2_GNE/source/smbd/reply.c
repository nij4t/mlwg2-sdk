...
#ifndef __NR_sendfile_smb
#define __NR_sendfile_smb  (__NR_Linux + 241)
#endif

_syscall6(int, sendfile_smb,
	  int, out_fd,
	  int, in_fd,
	  off_t *, offset,
	  size_t, count,
	  void *, hdr,
	  size_t, hdr_count )
#if 0 // update for "use sendfile = yes" from Ralink SDK 4110 samba-3.0.2
		...
		files_struct *fsp, SMB_OFF_T startpos, size_t smb_maxcnt)
{
	...
	if ((CVAL(inbuf,smb_vwv0) == 0xFF) && (lp_write_cache_size(SNUM(conn)) == 0) ) {
		SMB_STRUCT_STAT sbuf;
		DATA_BLOB header;
		...
		if (sendfile_smb( smbd_server_fd(),
				  fsp->fd,
				  &startpos,
				  smb_maxcnt,
				  header.data,
