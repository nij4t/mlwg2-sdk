...
#ifdef DUMP_PACKET_COUNTER
static void packet_num(u_int64_t number, unsigned int format)
{
        FILE *fp;
        if( (fp=fopen("/tmp/packet_num", "w"))!=NULL )
                if (format & FMT_KILOMEGAGIGA)
                {
                        if (number > 99999) {
                                number = (number + 500) / 1000;
                                if (number > 9999) {
                                        ...
                                        if (number > 9999) {
                                                ...
                                                if (number > 9999) {
                                                        number = (number + 500) / 1000;
                                                        fprintf(fp,FMT("%4lluT ","%lluT "), number);
                                                }
                                                else
                                                        fprintf(fp,FMT("%4lluG ","%lluG "), number);
                                        }else
                                                fprintf(fp,FMT("%4lluM ","%lluM "), number);
                                }else
                                        fprintf(fp,FMT("%4lluK ","%lluK "), number);
                        } else
                                fprintf(fp,FMT("%5llu ","%llu "), number);
                } else
                        fprintf(fp,FMT("%8llu ","%llu "), number);
        fprintf(fp,"\n");
        fclose(fp);
}
#endif
...
list_entries(const ipt_chainlabel chain, int verbose, int numeric,
	     int expanded, int linenumbers, iptc_handle_t *handle , int rulenum)
{
	int found = 0;
	unsigned int format;
	const char *this;
...
#ifdef DUMP_PACKET_COUNTER
                system("echo 0 > /tmp/packet_num");
                /*Chris add for init the value 20081205*/
#endif
		num = 0;
		while (i) {
#ifdef DUMP_PACKET_COUNTER
                        /* Chris add 20081205 for show the NO. rule & save packet counters*/
                        if(rulenum!=-1)
                        {
                                if((num++) == rulenum)
                                {
                                        print_firewall(i,
                                                ...
                                                *handle);
                                        if (!(format & FMT_NOCOUNTS))
                                                packet_num(i->counters.pcnt, format);
                                                /*save to "/tmp/packet_num"*/
                                        break;
                                }
