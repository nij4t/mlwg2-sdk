...
#define CHUNK 128
	...
	free(fdb);	// Ben, Coverity Run: 62 (2009-11-09 20:45) CID: 4464
	return 0;
}

#ifdef BLOCK_WLAN_FW
static int br_cmd_set_wlan_forward(int argc, char *const* argv)
{
	struct ifreq ifr;
	int err;
	int br_socket_fd = -1;
	unsigned long forward_mapping;

	forward_mapping = atoi(argv[2]); // bit0: ra0 bit1: ra1 ....
	...
	unsigned long args1[4] = { BRCTL_SET_WIRELESS_FORWARD, forward_mapping, 0, 0};
...
#ifdef BLOCK_WLAN_FW
	{ 2, "setwlanforward", br_cmd_set_wlan_forward,
	  "<bridge> <flag>\tset guest port bit0:ra0 bit3:ra3 0:enable"},
#endif
};
