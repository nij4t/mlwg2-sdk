                              ...
                              const char * minissdpdsock, int sameport)
{
	...
	/* Jayo 2009/11/13: device list exclude our device */
	char IPAddr[16];
	int RouterMode = atoi((char *)nvram_get("RouterMode"));
	int BrMethod = atoi((char *)nvram_get("BrMethod"));
	memset(IPAddr, 0x00, sizeof(IPAddr));
	if(RouterMode)
		strncpy(IPAddr, (char *)nvram_get("LanIPAddr"), sizeof(IPAddr));
	else if(BrMethod)
		strncpy(IPAddr, (char *)nvram_get("BrLanIPAddr"), sizeof(IPAddr));
	else
		strncpy(IPAddr, (char *)nvram_get("WanCurrentIP"), sizeof(IPAddr));
	...
	if (n < 0) {
		...
		if(st&&descURL)
		{
			...
			/* Jayo 2009/11/13: device list exclude our device */
			if(strstr(tmp->buffer, IPAddr) != NULL)
			{
				free(tmp);
				continue;
			}
